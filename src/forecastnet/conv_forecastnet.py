...
class ForecastNetConvModel(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, in_seq_length, out_seq_length, device):
        ...
        self.conv_layer3 = nn.ModuleList([nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=1) for i in range(out_seq_length)])
        ...
        # self.conv_layer3 = nn.ModuleList([nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=1) for i in range(out_seq_length)])
        # self.pool_layer3 = nn.ModuleList([nn.AvgPool1d(kernel_size=2, padding=0) for i in range(out_seq_length) for i in range(out_seq_length)])
    ...
    def forward(self, input, target=None, is_training=False):
        ...
        for i in range(self.out_seq_length):
            ...
            hidden = F.relu(self.conv_layer3[i](hidden))
            # hidden = self.pool_layer3[i](hidden)
